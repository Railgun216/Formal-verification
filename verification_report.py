...
def generate_report():
    ...
    report.append(f"✓ Star set created for ε=0.01")
    report.append(f"  - Center shape: {input_star.center.shape}")
    report.append(f"  - Basis shape: {input_star.basis.shape}")
    report.append(f"  - Lower bound range: [{lb.min():.4f}, {lb.max():.4f}]")
    report.append(f"  - Upper bound range: [{ub.min():.4f}, {ub.max():.4f}]")
    report.append("")
    
    # Test 4: Reachability
    report.append("[TEST 4] Reachability Analysis")
    report.append("-" * 80)
    output_star = verifier.reachability.reach(input_star)
    output_lb, output_ub = output_star.get_bounds()
    report.append(f"✓ Reachability analysis completed")
    report.append(f"  - Output bounds shape: {output_lb.shape}")
    report.append(f"  - Output range: [{output_lb.min():.4f}, {output_ub.max():.4f}]")
    report.append("")
    ...
    report.append("✓ Star abstraction working")
    report.append("✓ Reachability analysis functional")
